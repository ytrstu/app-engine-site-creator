...
#from core.content.models import Sidebar, Page, UserProfile, UserGroup, File, FileStore
...
def respond(request, template, params=None):
    ...
    if hasattr(request, 'profile') and request.profile is not None:
        ...
        params['sidebar'] = Sidebar.render(None)
...
def edit_updated_page(page_id, message_id='', tab_name=''):
    ...
    Args:
      ...
      message_id: the id of the message element to be displayed to the
      user once
                  the page is reloaded
      tab_name: the name of the tab to default to when the page is
      reloaded
    ...
    """
    url = urlresolvers.reverse('core.views.admin.edit_page', args=[str(page_id)])
    if message_id:
...
def memcache_get(key):
    ...
    This method is currently in place to avoid having to disable the 
    pylint message across the codebase.
...
def edit_instance(request, model_type, model_form_type,
    ...
    Args:
      ...
      success_url: the URL to redirect the user to when the editing is 
      succesful
      object_id: the ID of the object to edit, or None if creating a new 
      object
      kwargs: additional data to be passed to the edit form

    Returns:
      A HTTP response, either a redirect to the success_url or the edit
      form.
    ...
    if object_id:
        ...
        if type_instance is None:
            return http.HttpResponseNotFound('No object exists with'
                                             'key %r',
                                             object_id)
...
def set_up_data_store():
    ...
    """
    from core.models.files import AccessControlList, Page
    acl = AccessControlList(global_read=True)
    acl.put()
    root = Page(name='Home', title='Welcome to App Engine Site Creator')
    root.acl = acl
    root.put()
    return root
